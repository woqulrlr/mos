...
void PerformSuperpixelSLIC(double* lvec, double* avec, double* bvec, double* kseedsl, double* kseedsa, double* kseedsb, double* kseedsx, double* kseedsy, int width, int height, int numseeds, int* klabels, int STEP, double compactness)
{
    ...
    for( k = 0; k < numk; k++ ){
    	 printf("the %d point: ",k);
         printf("x = %f",kseedsx[k]);
         printf("y = %f\n",kseedsy[k]); 
    }
                 ...
                 int nrhs, const mxArray *prhs[])
{
    ...
    plhs[2] = mxCreateNumericMatrix(1,numseeds,mxINT32_CLASS,mxREAL);
    outxs = (double*)mxGetData(plhs[2]);
    
    for(i1 = 0; i1<numseeds; i1++)
    {
        outxs[i1] = kseedsx[i1];
    }

    
    plhs[3] = mxCreateNumericMatrix(1,numseeds,mxINT32_CLASS,mxREAL);
    outys = (double*)mxGetData(plhs[3]);
    for(i1 = 0; i1<numseeds; i1++){
        outys[i1] = kseedsy[i1];
    }
